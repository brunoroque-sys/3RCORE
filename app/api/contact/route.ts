  ...
  try {
      ...
      html: `
        <div style="font-family: 'Helvetica', sans-serif; background-color: #130218; padding: 40px; color: white;">
          <div style="max-width: 600px; margin: 0 auto; background-color: #1a0522; border-radius: 24px; padding: 40px; border: 1px solid #ffffff10;">
            ...
            <p style="color: #b3b3b3; font-size: 16px;">Alguien se ha interesado en lo que hacemos. Aquí tienes los detalles:</p>
            
            <div style="background: rgba(255,255,255,0.05); border-radius: 16px; padding: 25px; margin: 25px 0;">
              <p style="margin: 10px 0;"><strong>Persona:</strong> ${nombre} ${apellido}</p>
              <p style="margin: 10px 0;"><strong>Correo:</strong> <a href="mailto:${email}" style="color: #9C27B0; text-decoration: none;">${email}</a></p>
            ...
            <p style="font-size: 12px; color: #666; text-align: center;">Enviado desde el formulario de 3rcore.app</p>
          </div>
        </div>
      `,
    });
    ...
    try {
      await resend.emails.send({
        ...
        html: `
          <div style="font-family: 'Helvetica', sans-serif; background-color: #f4f4f7; padding: 40px; color: #1a1a1a;">
